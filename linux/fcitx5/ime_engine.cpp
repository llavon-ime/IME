...
void ImeEngine::keyEvent(const fcitx::InputMethodEntry&, fcitx::KeyEvent& event) {
    ...
    if (key == FcitxKey_Return && !buffer_.empty()) {
...
void ImeEngine::reload_config() {
    ...
    const bool has_legacy_config = std::filesystem::exists(legacy_config_path(), ec) && !ec;
    if (has_fcitx_config || has_legacy_config) {
...
std::optional<int> ImeEngine::selection_index_for_key(fcitx::KeySym key) const {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (normalized_key == selection_key)
            return i;
    }
    return std::nullopt;
}
...
FCITX_ADDON_FACTORY(ime::linux::fcitx5::ImeEngineFactory)
