...
void apply_fallback_to_last_segment(ime::linux::CompositionBuffer& buffer, const ime::linux::FallbackEngine& fallback) {
...
bool type_keys(ime::linux::CompositionBuffer& buffer, const ime::linux::FallbackEngine& fallback,
               const std::u32string& keys) {
    for (const char32_t key : keys) {
        const auto mapped = ime::linux::lookup_bopomofo_key(key);
        if (!mapped || !buffer.add_bopomofo(*mapped)) return false;
        if (ime::linux::is_bopomofo_tone(*mapped)) apply_fallback_to_last_segment(buffer, fallback);
    }
    return true;
}
...
int run_buffer_tests() {
    ...
    ime::linux::CompositionBuffer buffer;
    ok = ok && buffer.empty();
    ...
    ok = ok && buffer.add_bopomofo(U'ㄧ');
    ok = ok && buffer.add_bopomofo(U'ˇ');
    ime::linux::FallbackEngine fallback("tables/bopomofo_char.json");
    const auto predictions = fallback.predict(buffer);
    ...
    const auto target = buffer.candidate_target(ime::linux::CandidateTarget::BeforeCursor);
    ...
    ok = ok && buffer.candidate_target(ime::linux::CandidateTarget::BeforeCursor) &&
         *buffer.candidate_target(ime::linux::CandidateTarget::BeforeCursor) == 0;
    ok = ok && buffer.candidate_target(ime::linux::CandidateTarget::AfterCursor) &&
         *buffer.candidate_target(ime::linux::CandidateTarget::AfterCursor) == 1;
