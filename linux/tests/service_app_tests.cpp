...
int run_service_app_tests() {
    ...
    ime::linux::ServiceApp app(cfg, "tables/bopomofo_char.json");
    ...
    try {
        ime::linux::ServiceApp status_only(cfg, "/tmp/ime-linux-missing-table.json");
    ...
    ime::linux::ServiceApp idle_app(cfg, "tables/bopomofo_char.json");
