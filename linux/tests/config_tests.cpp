...
int run_config_tests() {
    ...
    auto json = ime::linux::to_json(cfg);
    auto roundtrip = ime::linux::config_from_json(json);
    ...
    ok = ok && ime::linux::socket_path().filename() == "ime.sock";
    ok = ok && ime::linux::pid_path().filename() == "service.pid";
    ...
    ok = ok && ime::linux::config_path() == "/tmp/ime-home/.config/fcitx5/conf/ime-linux.conf";
    ok = ok && ime::linux::legacy_config_path() == "/tmp/ime-home/.config/ime-linux/config.json";
    ok = ok && ime::linux::runtime_dir() == std::filesystem::temp_directory_path() / "ime-linux";

    const auto config_root = std::filesystem::temp_directory_path() / "ime-linux-config-test";
    std::filesystem::remove_all(config_root);
    setenv("XDG_CONFIG_HOME", config_root.c_str(), 1);
    std::filesystem::create_directories(ime::linux::config_path().parent_path());
    {
        std::ofstream output(ime::linux::config_path());
        output
    ...
    const auto loaded = ime::linux::load_config();
...

    {
        std::ofstream output(ime::linux::config_path());
        output
            << "BopomofoKeyboardLayout=標準\n"
            << "SelectionKeys=左手鍵\n"
            << "CandidateLayout=垂直\n"
            << "SelectPhrase=游標後\n"
            << "CapsLockInputsBopomofo=True\n";
    }
    const auto chinese_loaded = ime::linux::load_config();
    ...
    const auto invalid = ime::linux::config_from_json(nlohmann::json::parse(
    ...
    ok = ok && invalid.selection_keys == ime::linux::default_config().selection_keys;
    ok = ok && invalid.selection_key_count == ime::linux::default_config().selection_key_count;
    ok = ok && invalid.candidate_page_size == ime::linux::default_config().candidate_page_size;
    ok = ok && invalid.candidate_layout == ime::linux::default_config().candidate_layout;
    ok = ok && invalid.space_selects_candidate == ime::linux::default_config().space_selects_candidate;
    ok = ok && invalid.select_phrase == ime::linux::default_config().select_phrase;
    ok = ok && invalid.move_cursor_after_selection;
    ok = ok && invalid.caps_lock_inputs_bopomofo == ime::linux::default_config().caps_lock_inputs_bopomofo;

    std::filesystem::remove(ime::linux::config_path());
    std::filesystem::create_directories(ime::linux::legacy_config_path().parent_path());
    {
        std::ofstream output(ime::linux::legacy_config_path());
        output << R"({"model_path":"/tmp/legacy.gguf","gpu_layers":7})";
    }
    const auto legacy_loaded = ime::linux::load_config();
    ...
    try {
        const auto malformed = ime::linux::load_config();
        malformed_uses_default =
            malformed.model_path.empty() && malformed.context_length == ime::linux::default_config().context_length;
    } catch (...) {
