...
std::optional<std::filesystem::path> resolved_service_path() {
    ...
    for (const auto& path : {std::filesystem::path("/usr/local/bin/ime-linux-service"),
                            std::filesystem::path("/usr/bin/ime-linux-service")}) {
        if (std::filesystem::exists(path)) return path;
    }
...
PredictState ServiceClient::request_predict_async(PredictRequest request, std::function<void(PredictState)> on_complete) {
    {
