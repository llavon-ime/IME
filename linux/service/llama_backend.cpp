...
#ifndef IME_LINUX_HAS_LLAMA
#define IME_LINUX_HAS_LLAMA 0
#endif

#if IME_LINUX_HAS_LLAMA
#include <llama.h>
#endif
...
std::filesystem::path tables_dir() {
    if (const char* override = std::getenv("IME_LINUX_TABLE_DIR")) {
    ...
    std::vector<int> tokenize(const PredictRequest& request) const {
        ...
        for (const auto& entry : request.padding) {
            if (entry.chosen) {
                ...
                if (it == bopomofo_table_.end()) throw std::runtime_error("invalid bopomofo token: " + key);
...
void LlamaBackend::load(const Config& cfg) {
    ...
    try {
        ...
        if (impl_->context == nullptr) throw std::runtime_error("failed to create llama.cpp context: " + cfg.model_path);
