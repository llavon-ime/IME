...
ime::fcitx5::ByteVector recv_message(const ime::fcitx5::UnixSocketConnection& connection) {
...
void send_status(const ime::fcitx5::UnixSocketConnection& connection, ime::fcitx5::ServiceApp& app) {
    connection.send_all(ime::fcitx5::encode_message(app.handle_status()));
}

void handle_connection(const ime::fcitx5::UnixSocketConnection& connection, ime::fcitx5::ServiceApp& app) {
    const auto message = recv_message(connection);
    const auto type = ime::fcitx5::decode_message_type(message);

    if (type == "control_request") {
        const auto request = ime::fcitx5::decode_control_request(message);
    ...
    if (type == "predict_request") {
        const auto request = ime::fcitx5::decode_predict_request(message);
        connection.send_all(ime::fcitx5::encode_message(app.handle_predict(request)));
        return;
    }

    ime::fcitx5::StatusResponse status = app.handle_status();
    status.error = "unknown message type: " + type;
    connection.send_all(ime::fcitx5::encode_message(status));
}
...
int run(int argc, char** argv) {
    ...
    server.bind_listen(ime::fcitx5::socket_path());
